...
- 涉及任何样式调整或界面设计改动前，必须先阅读 `specs/DESIGN.md` 并严格遵守其中要求；如果目标改动与 `specs/DESIGN.md` 存在冲突或矛盾，先和 Owner 确认，不要自行覆盖设计约束。
- 轻背景、克制配色、清晰排版、大留白。
...
- 对计算过程、定位逻辑、状态同步、事件时序等不容易一眼看懂的复杂逻辑，添加适量但足够具体的注释，说明关键约束、为什么这样判断，以及哪些边界条件会影响结果；不要只复述代码表面动作。
