...
export const SelectedProjectsList = async ({ lang, copy }: { lang: string, copy: Dictionary['ui']['projects'] }) => {
  const selectedProjects = await getSelectedProjects(lang);
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 lg:gap-10 px-4 lg:px-0" role="list" dev-mode="tailwind">
      {selectedProjects.map(({ id, title, desc, github, image, hoveredImage }) => (
        <div key={id} className="w-full max-w-md lg:max-w-none mx-auto">
