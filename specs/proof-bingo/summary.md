...
- 完成态不重复渲染 View Projects / Hire Me CTA；这两个转化入口由首页 hero 左侧常驻 CTA 承担。
- 完成态 Reset：
  - EN: `Reset`
  - ZH: `重置`
  - 位置：完成态区域内，作为低权重 secondary action。
  - 行为：清空已选 tile、完成线和完成态，回到初始 bingo 状态。
- CTA：`Hire Me` 是首页 hero CTA 文案，不新增独立 `/hire-me` 页面。
...
- 语气：身份总结偏 playful、有趣。
- 移动端：保留完整 3x3 互动。
- 移动端布局：3x3 始终完整显示，不改轮播/列表；tile 使用稳定尺寸或 `aspect-ratio: 1 / 1`；文案保持短句并允许换行，不在 tile 内放详细解释；完成态 Reset 需适合触控。
- 文案语言：身份总结中英双语。
